...
if os.host() == "windows" then
    if VulkanSDK == nil or VulkanSDK == "" then
        error("VULKAN_SDK wurde nicht gefunden.\nInstalliere das LunarG Vulkan SDK und starte das Terminal neu.")
    end

    local vulkanHeader = path.join(VulkanSDK, "Include", "vulkan", "vulkan.h")
    local vulkanLibrary = path.join(VulkanSDK, "Lib", "vulkan-1.lib")

    if not os.isfile(vulkanHeader) then
        error("Der Vulkan-Header wurde nicht gefunden:\n" .. vulkanHeader)
    end

    if not os.isfile(vulkanLibrary) then
        error("Die Vulkan-Bibliothek wurde nicht gefunden:\n" .. vulkanLibrary)
    end
end
...
if VulkanSDK ~= nil and VulkanSDK ~= "" then
    IncludeDir["VulkanSDK"] = path.join(VulkanSDK, "Include")
